...
func Watch(name string, consumed int, s *service.Service, e *environment.Environment, startTime *int64, config *aws.Config, log *logger.Logger) error {
	...
	for endsWithInProgress {
		...
		if !endsWithInProgress && strings.Contains(*response.Stacks[0].StackStatus, "ROLLBACK") {
			return fmt.Errorf("There was an error and the stack is '%s'", *response.Stacks[0].StackStatus)
		}
	}
	return nil
}
